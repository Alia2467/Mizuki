...
| 安全防护 | API 速率限制（滑动窗口按 IP 限流），`/phone-data` 与 `/merged-data` 1200 次/分（适配实时上报与历史补传），导出 12 次/分，超限返回 429 |
| 控制台鉴权 | `/api/*`、`/phone-data`、`/merged-data`、`/health/deep` 统一受 `X-Sensor-Token` 保护；配置接口不返回令牌原值；WebUI 令牌仅限当前标签页 |
...
| `plugin_heartbeat_timeout` | 插件心跳超时（秒），超过此时间未心跳视为离线，默认 `10` |
...
```bash
# 运行自动化测试（130+ 个用例：服务端 64 + 插件 133 + 前端 28）
py -m pytest test/ -v
